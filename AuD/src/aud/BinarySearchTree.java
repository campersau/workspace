...
/** Base class for a binary search tree.<p>
    ...
    <li>Keep in mind that this is intended to be an
    <em>educational</em> implementation and not an optimized one!
    </li>

    <li>The internal node ({@link BinarySearchTree.Node}
    representation is based on {@link BinaryTree}!<p> On the one hand,
    this choice enables us to reuse functionality. On the other hand,
    there are some <em>disadvantages</em> due to extra data that is
    stored or extra indirections for method calls.</li>

    <li>Each {@link BinarySearchTree.Node} stores a <em>key-value</em>
    pair of type {@link BinarySearchTree.Entry} as node data.</li>

    <li>{@code BinarySearchTree} stores <em>head</em> as a <em>dummy
    node</em> (member {@code head_}), which has has by definition a
    smaller key than any other node.</li>

    <li>This implementation does <em>not</em> use dummy nodes as
    leaves!</li>

    <li>The core function used for finding ({@link #find}) a node by
    key <em>and</em> for finding an insertion point (for {@link
    #insert}) is {@link #findLowerBound}). It returns a {@link
    BinarySearchTree.LowerBound} instance.</li>

    <li>Keys are compared by {@link #compareKeys}, which treats {@code
    null} keys as smallest possible keys. Then <em>head</em>'s key
    equals {@code null} by definition.</li>

    <li>{@link BinarySearchTree.Cursor} provides a reference to node
    in the tree, i.e., allows access to its key and value. The {@code
    iterator()} method implements {@code Iterable<Cursor>}, and this
    <em>inorder traversal</em> iterator provides {@code Cursor}
    instances. (With some more work, we could alternatively provide
    different iterators over keys, values, and key-value
    pairs/entries.)</li>

    <li>{@link #insert} provides a callback {@link #onInsert} to
    notify a subclass of a change of the tree. Similarly, {@link #remove}
    invokes {@link #onRemove}.</li>

    <li>Note that {@link BinaryTree#getLeft} and {@link BinaryTree#getRight} 
    also update links to parents! While this is convenient in many cases, it 
    requires special care in other cases! (Explicitly setting {@code null} 
    references, order of operations matters "even more".)</li>

    <li>Although this is <em>not</em> a balanced tree, {@code BinarySearchTree}
    ...
    <li> {@link #rotateLeft} single rotation, right-left case</li> 
    <li> {@link #rotateRight} single rotation, left-right case </li> 
    </ul>    
    ...
    <li>Note that we excessively use the fact that an extra reference
    to the node's parent is stored. In fact, this allows for a simple
    ...
    of a more complex implementation!</li>
...
public class BinarySearchTree<Key extends Comparable<Key>,Value> 
  ...
  */
  ...
  /** Restructure subtree following <b>Goodrich&Tamassia</b> (2nd ed, p. 269).
  ...
  /** Apply left rotation (right-left case).
      <pre><code>
         a                     b
       /   \                 /   \
      t0    c      ==>     a       c
  ...
  /** Apply right rotation (left-right case).
      <pre><code>
                c                    b
              /   \                /    \
             b    t3     ==>     a        c
